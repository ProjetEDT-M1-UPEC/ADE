...
/**
 * 
 * AgendaCustom est une classe heritant de la classe Agenda de la bibliotheque
 * Jfxtras.
 * 
...
public class AgendaCustom extends Agenda {
	 ...
	 * 
	 * @param t Emploi du temps que l'agenda va contenir
	 */
	public AgendaCustom(TimeTable t) {
		...
		this.setAppointmentChangedCallback(new Callback<Agenda.Appointment, Void>() {
			...
			public Void call(Appointment ap) {
				...
				if (isDraged)
					param.notify(new AgendaEvent(param, AgendaEvent.TYPE.DATE,
							new PairDate((Calendar) param.getOld_StartCal().clone(),
									(Calendar) param.getOld_EndCal().clone()),
							new PairDate((Calendar) param.getStartTime().clone(),
									(Calendar) param.getEndTime().clone())));
				
				Creneaux clone = (Creneaux) param.clone();
				clone.back();
	 ...
	 * 
	 * @param timeTable Met a jour l'emploi du temps
	 */
	...
	private void agandaProp() {
		this.newAppointmentCallbackProperty().set((localDateTimeRange) -> {
			...
			c.setParent(parent);
			c.notify(new AgendaEvent(c, AgendaEvent.TYPE.ADD, new PairDate(null, null),
					new PairDate((Calendar) c.getStartTime().clone(), (Calendar) c.getEndTime().clone())));
	...
	/**
	 * Afficher le Pop up d'edition d'un creneau
	 * 
	 * @param cr
	 * @throws IOException
	 */
	private void editPopUp(Appointment cr) throws IOException {
		...
		FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource(Constants.PopUpCR_FXML));
	 ...
	 * 
	 * @param timeTable2 Nouvel emploi du temps qui remplace le precedant
	 */
	...
	private static boolean compareNullableString(String str, String filterStr) {
		...
		return (filterStr == null || filterStr.trim().equals("")) ? true
				: str.toUpperCase().contains(filterStr.toUpperCase());
	 ...
	 * 
	 * @param cours Valeur contenu dans le champ cours du formulaire
	 * @param prof  Valeur contenu dans le champs prof du formulaire
	 * @param salle Valeur contenu dans le champs salle du formulaire
	 * @param group Valeur contenu dans le champs groupe du formulaire
	 */
	public void filter(String cours, String prof, String salle, String group) {
		List<Appointment> list = appointments().stream()
				...
				.filter(e -> compareNullableString(((Creneaux) e).getGroup(), group)).collect(Collectors.toList());
		this.appointments().clear();
