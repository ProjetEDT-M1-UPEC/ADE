...
public class Tab extends javafx.scene.control.Tab {
	 ...
	 * @param parent Noeud parent auquel est rattache le Tab
	 */
	...
	public void setVersionId(String id) {
		versionId = id;
		agenda.getTimeTable().setVersionId(id);
	}

	/**
	 * Constructeur de la classe Tab
	 * 
	 * @param agenda Agenda affiche dans l'onglet
	 * @param parent Noeud parent auquel est rattache le tab
	 */
	public Tab(AgendaCustom agenda, MainScreenControleur parent) {
		...
		next = new UndoRedoManager<AgendaEvent>(limit);
		this.parent = parent;
		versionId = agenda.getTimeTable().getVersionId();
	}
	...
	public void contextMenu() {
		...
		closeRightItems.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent event) {
				...
				MainScreenControleur.tabPaneV2.getTabs().remove(
						MainScreenControleur.tabPaneV2.getTabs().indexOf(me) + 1,
	 ...
	 * 
	 * @param agenda Met a jour l'agenda
	 */
