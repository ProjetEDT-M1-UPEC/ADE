...
public class JsonFileManager implements FileManager {
	...
	public TimeTable load(File file) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, modeles.Constants.errLoadFile + e.getMessage(),
	...
	public boolean save(TimeTable tm, String path) {
		...
		try {
			...
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, modeles.Constants.errSaveFile + e.getMessage(),
					modeles.Constants.errMssg, JOptionPane.ERROR_MESSAGE);
			return false;
		}
		return true;
	...
	public Version2 loadVersion(File file) {
		...
		try {
			...
			Version.setRootName(file.getName().replaceAll(".json", ""));
		} catch (MismatchedInputException e1) {
			JOptionPane.showMessageDialog(null, modeles.Constants.errLoadFile + MismatchFile, modeles.Constants.errMssg,
					JOptionPane.ERROR_MESSAGE);
			return null;
